...
Use `intermesh profile plan` against the Codex catalog first. V0 permits activation only when every removable entry is a symlink; regular directories make the plan `routing_only`. `profile apply --yes` snapshots the exact links before replacing them with `intermesh-router`, and `profile restore --yes` reverses the operation.
